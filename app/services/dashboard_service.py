...
#         pending = BGVRequest.query.filter_by(status="Initiated", is_deleted=False).count()
#         completed = BGVRequest.query.filter_by(status="Completed", is_deleted=False).count()
#         rejected = BGVRequest.query.filter_by(final_decision="Rejected", is_deleted=False).count()
