...
# @document_bp.route("/candidate/upload-document", methods=["POST"])
# def upload_document():
#     print("DOCUMENT ROUTE HIT")
#     try:
#         secure_token = request.form.get("secure_token")

#         document_type = request.form.get("document_type")

#         file = request.files.get("file")

#         result = DocumentService.upload_document(secure_token, document_type, file)
...
#         full_path = os.path.join(project_root, document["file_path"])
...
def view_document(document_id):
    ...
    try:
        ...
        if not file_path:
            return jsonify(
                {"status": "error", "message": "Document file path is missing"}
            ), 404
